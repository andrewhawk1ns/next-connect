...
class UserFeed extends React.Component {
  ...
  handleFollow = (user, userIndex) => {
    followUser(user._id).then(user => {
      const updatedUsers = [
        ...this.state.users.slice(0, userIndex), //get everything before the users index
        ...this.state.users.slice(userIndex + 1) //get everything after the users index
      ]
      this.setState({
        users: updatedUsers,
        openSuccess: true,
