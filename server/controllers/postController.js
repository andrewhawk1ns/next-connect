...
exports.getPostFeed = async (req, res) => {
    ...
    following.push(_id); //Add the user's own id to the following users
...
exports.toggleLike = async (req, res) => {
    ...
    const likeIds = post.likes.map(id => id.toString());  //_id's aren't strings we need to convert to a string
    const authUserId = req.user._id.toString();
    if (likeIds.includes(authUserId)) {
        await post.likes.pull(authUserId); //Remove the authenticated user's like for unlike
    } else {
        await post.likes.push(authUserId); //Add the authenticated user's like for like
    }
    await post.save();
    res.json(post);
};
